...
export class NormalZombie extends Zombie {
  health = ZOMBIE_DETAILS.zombie.health
  speed = ZOMBIE_DETAILS.zombie.speed

  start(): void {
    const sprite = new Sprite({
      sprite: AssetManager.get('normal-zombie-walking')!,
      columns: 4,
      rows: 2,
    })
  ...
  update(dt: number): void {
    ...
    // Health
    if (!this.hasHalfLife && this.health <= ZOMBIE_DETAILS.zombie.health / 2) {
